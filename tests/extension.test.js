...
import { buildAssetManifest, createAssetReport, discoverAssets, getNodeRole, getNodeTag, selectAssetsForProcessing } from '../extension/src/assets.js';
...
import { buildElementorAjaxBody, buildElementorSavePayload, ensureWordPressDraft, extractWordPressContext, insertElementorDocument, validateWordPressContext, verifyElementorPersistence } from '../extension/src/wordpress.js';
...
test('extension discovers tagged image, background and SVG icon assets', () => {
  ...
  assert.deepEqual(assets.map(asset => asset.kind), ['image', 'background', 'icon']);
...
test('extension creates an Elementor document and patches uploaded media IDs', async () => {
  ...
  assert.equal(patched.figmentor.elements[patched.content[0].elements[0].id].assets.image.status, 'uploaded');
});
...
test('extension flattens page wrapper output and maps backgrounds to native containers', async () => {
  ...
  assert.equal(document.figmentor.elements[document.content[0].id].assets.background_image.assetRef, 'figmentor-3-2-background');
...
test('REST traversal preserves nested layout, spacing, typography, color, border and shadow', async () => {
  const root = {
    ...
    strokeWeight: 2,
    cornerRadius: 16,
    effects: [{ type: 'DROP_SHADOW', visible: true, offset: { x: 0, y: 8 }, radius: 20, spread: 2, color: { r: 0, g: 0, b: 0, a: 0.4 } }],
    pluginData: { [pluginId]: { 'elementor-tag': 'container' } },
    children: [{
      id: '10:2',
      name: 'Content row',
      ...
      children: [{
        ...
        style: { fontFamily: 'Inter', fontSize: 52, fontWeight: 700, textAlignHorizontal: 'CENTER', lineHeightPx: 60, letterSpacing: -1 },
...
test('Font Awesome remains native and custom vectors remain real SVG assets', async () => {
  const fontAwesomeRoot = {
    ...
    children: [
      { id: '11:2', name: '[ICON] fas fa-check', type: 'VECTOR', pluginData: { [pluginId]: { elementor_role: 'icon' } }, fills: [] },
      { id: '11:3', name: '[TITLE] Item', type: 'TEXT', characters: 'Item', pluginData: { [pluginId]: { elementor_role: 'title_text' } } }
    ]
  };
  const customRoot = {
    ...fontAwesomeRoot,
    ...
    children: [
      { id: '12:2', name: '[ICON] Arrow custom', type: 'VECTOR', pluginData: { [pluginId]: { elementor_role: 'icon' } }, fills: [] },
      { id: '12:3', name: '[TITLE] Item', type: 'TEXT', characters: 'Item', pluginData: { [pluginId]: { elementor_role: 'title_text' } } }
    ]
  };
  ...
  assert.equal(customDocument.figmentor.elements[customDocument.content[0].id].assets.selected_icon.assetRef, 'figmentor-12-2-icon');

  const patched = patchElementorAssets(customDocument, {
    assets: [{ ...customAssets[0], status: 'uploaded', mediaId: 77, mediaUrl: 'https://site.test/arrow.svg' }]
  });
...
test('failed custom icon-list SVGs use a valid explicit Font Awesome placeholder', async () => {
  const root = {
    ...
    children: [
      { id: '13:2', name: '[ICON] Custom arrow', type: 'VECTOR', pluginData: { [pluginId]: { elementor_role: 'icon' } }, fills: [] },
      { id: '13:3', name: '[TITLE] Primeiro item', type: 'TEXT', characters: 'Primeiro item', pluginData: { [pluginId]: { elementor_role: 'title_text' } } },
      { id: '13:4', name: '[ICON] Custom star', type: 'VECTOR', pluginData: { [pluginId]: { elementor_role: 'icon' } }, fills: [] },
      { id: '13:5', name: '[TITLE] Segundo item', type: 'TEXT', characters: 'Segundo item', pluginData: { [pluginId]: { elementor_role: 'title_text' } } }
    ]
  };

  const document = await buildElementorDocument(root, 'page', pluginId);
  const manifest = buildAssetManifest(root, pluginId, { fileKey: 'file', nodeId: '13:1', name: root.name });
  const prepared = document.content[0].settings.icon_list;

  assert.deepEqual(prepared.map(item => item.selected_icon), [
  ...
  assert.deepEqual(patched.content[0].settings.icon_list.map(item => item.selected_icon), [
    { value: 'fas fa-check', library: 'fa-solid' },
    { value: 'fas fa-check', library: 'fa-solid' }
  ]);
  assert.equal(validateElementorDocument(patched, 'page').valid, true);
  assert.equal(patched.figmentor.elements[patched.content[0].id].assets.icon_list[0].status, 'failed');
});
...
test('asset failures produce a detailed report and retry selects only failed assets', () => {
  const manifest = {
    assets: [
      { assetRef: 'ok', elementName: 'Logo', figmaNodeId: '1:1', kind: 'image', status: 'uploaded', mediaId: 1, mediaUrl: 'https://site.test/logo.webp' },
      { assetRef: 'fail', elementName: 'Background', figmaNodeId: '1:2', kind: 'background', targetFormat: 'WEBP', status: 'failed', error: 'Acima de 150 KB' }
    ]
  };
  const retry = selectAssetsForProcessing(manifest, true);
  const report = createAssetReport(manifest);

  assert.deepEqual(retry.map(asset => asset.assetRef), ['fail']);
  ...
  const result = await convertPngBlobToWebp(new Blob(['png']), {
    ...
    canvasFactory: (width, height) => ({
      ...
      async convertToBlob({ quality }) {
        return new Blob([new Uint8Array(Math.round(width * height * quality / 20))], { type: 'image/webp' });
      }
    })
  });
...
test('Elementor AJAX body uses the real elementor_ajax envelope and keeps draft status', () => {
  ...
  const body = new URLSearchParams(buildElementorAjaxBody(
    context,
    'figmentor_save_builder',
    'save_builder',
    { status: 'draft', elements: [], settings: {} }
  ));
      ...
      executeScript: async ({ func, args }) => {
        ...
        globalThis.fetch = async (url, options) => {
          ...
          return {
            ok: true,
            status: 200,
            async json() { return { id: 42, status: 'draft' }; }
          };
        };
...
test('Elementor page mode handles an empty page and persistence verifies elements and media', async () => {
  ...
  const patched = patchElementorAssets(document, {
    assets: [{ assetRef: 'figmentor-13-1-image', status: 'uploaded', mediaId: 99, mediaUrl: 'https://site.test/hero.webp' }]
  });
...
test('Elementor save payload preserves existing elements in section mode', async () => {
  ...
  assert.equal(payload.status, 'draft');
  assert.deepEqual(payload.elements.map(element => element.id), ['existing-section', validDocument.content[0].id]);
      ...
      executeScript: async ({ func, args }) => {
        ...
        globalThis.fetch = async () => {
          const responseBody = requestCount === 1
            ? { id: 42, status: 'draft' }
            : requestCount === 2
              ? { success: true, data: { responses: { figmentor_get_document: { success: true, data: { elements: [], settings: {}, status: 'draft' } } } } }
              : { success: true, data: { responses: { figmentor_save_builder: { success: false, data: 'Falha simulada no Elementor.' } } } };
          return {
            ok: true,
            status: 200,
