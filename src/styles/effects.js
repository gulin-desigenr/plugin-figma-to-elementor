...
import { getSelectorDefinition, resolveItemSelector, resolveSelector } from "./elementor-selectors.js";

const CSS_SAFE_PROPERTIES = new Set([
  "background", "background-image", "background-blend-mode", "mix-blend-mode",
  "filter", "backdrop-filter", "-webkit-backdrop-filter", "box-shadow",
  "border-image", "opacity", "color", "background-clip", "-webkit-background-clip"
]);
...
function declaration(property, value) {
  if (!CSS_SAFE_PROPERTIES.has(property) || value === null || value === undefined || value === "") return null;
...
export function extractAdvancedEffects(node, widgetType, cssId) {
  ...
  const hasAdvancedEffect = effects.length > 1 || effects.some(effect => effect.type !== "DROP_SHADOW") ||
    (node?.opacity !== undefined && finite(node.opacity, 1) < 1) || (blendMode && blendMode !== "normal") || hasUnsupportedBlend;
  const native = {
    background: null,
  ...
  if (node?.opacity !== undefined && finite(node.opacity, 1) < 1) cssDeclarations.push(declaration("opacity", String(Math.max(0, Math.min(1, finite(node.opacity, 1))))));
  ...
  const itemSelectors = definition?.itemTarget && cssId && Array.isArray(node?.children)
    ? node.children.map((_, index) => resolveItemSelector(widgetType, cssId, index + 1)).filter(Boolean)
    : [];
  const resolvedSelector = cssId ? resolveSelector(widgetType, cssId, targetSlot) : null;
  const customCss = cssDeclarations.length && cssId
